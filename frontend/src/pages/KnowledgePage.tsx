  ...
  if (!data || data.nodes.length === 0) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center text-center">
        ...
        <h2 className="text-2xl font-bold text-white">Your Knowledge Graph is empty</h2>
        <p className="mt-2 text-slate-400 max-w-md">Start taking notes or generating roadmaps to see how your skills and knowledge interconnect.</p>
      </div>
    );
  }
    ...
    <div className="h-[calc(100vh-120px)] flex flex-col space-y-6 animate-fade-in overflow-hidden relative">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between relative z-10">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Visualize the neural map of your learning journey</p>
        </div>

        <div className="flex items-center gap-2 bg-white/[0.03] border border-white/10 rounded-2xl p-1">
          <button onClick={() => setZoom(prev => Math.min(2, prev + 0.1))} className="p-2 text-slate-400 hover:text-white transition"><Maximize2 size={18} /></button>
          <button onClick={() => setZoom(prev => Math.max(0.5, prev - 0.1))} className="p-2 text-slate-400 hover:text-white transition"><Minimize2 size={18} /></button>
        </div>
      </header>

      <div 
        ref={containerRef}
        className="flex-1 rounded-[2.5rem] border border-white/10 bg-black/40 overflow-hidden relative cursor-grab active:cursor-grabbing"
      >
                ...
                <div className="flex items-center justify-center h-full">
                  ...
                  >
                    ...
                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-20">
                      <span className="bg-black/80 text-[10px] text-white px-2 py-1 rounded border border-white/10">
                        {node.label}
                      </span>
                    </div>
        ...
        <AnimatePresence>
          {selectedNode && (
            <motion.aside
              ...
              className="absolute top-6 right-6 bottom-6 w-80 bg-panel/95 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-6 shadow-2xl z-30"
            >
              <button 
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="flex-1 space-y-6 overflow-y-auto pr-2 custom-scrollbar">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-xs font-mono text-white">{selectedNode.val}</span>
                    </div>
                  </div>
                ...
                <div className="mt-6 pt-6 border-t border-white/5">
                  <button className="w-full py-3 rounded-xl bg-brand text-sm font-bold text-white hover:bg-brand/90 transition shadow-lg shadow-brand/20">
                    Open Node Details
                  </button>
                </div>
              </div>
