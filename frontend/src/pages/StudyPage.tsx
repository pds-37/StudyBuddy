  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
      <header className="flex items-center justify-between">
        <Link to="/dashboard" className="group flex items-center gap-2 text-slate-500 text-slate-500 text-slate-400 hover:text-white text-white text-white transition">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        <div className="space-y-8">
          {/* Main Task Card */}
          <div className="rounded-3xl border border-white/10 border-white/10 border-white/10 bg-white/[0.03] p-8 lg:p-10 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h1 className="mt-1 text-3xl font-bold text-white text-white text-white">{task.title}</h1>
                </div>
              </div>

              <div className="mt-8 flex flex-wrap gap-6 text-sm text-slate-500 text-slate-500 text-slate-400">
              ...
              <div className="mt-8 space-y-4">
                <h3 className="text-sm font-semibold text-white text-white text-white uppercase tracking-wider">Mission Objective</h3>
                <p className="text-lg leading-relaxed text-slate-300 text-slate-300 text-slate-300">
                  {task.description}
                </p>
              </div>
              ...
              <div className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-white/[0.02] border border-white/5 p-5">
                <div>
                  ...
                  <p className="text-sm italic text-slate-500 text-slate-500 text-slate-400">"{task.reason}"</p>
                </div>
                {task.href && task.href !== location.pathname && (
                  <Link
                    to={task.href}
                    className="inline-flex items-center gap-2 rounded-xl border border-white/10 border-white/10 border-white/10 bg-transparent bg-transparent bg-white/5 px-5 py-2.5 text-xs font-bold text-white text-white text-white transition-all hover:bg-transparent bg-transparent bg-white/10 hover:border-white/20"
                  >
                    Open in Tool
          ...
          <div className="rounded-3xl border border-brand/20 bg-brand/[0.02] p-8 text-center relative overflow-hidden">
            ...
            <h2 className="text-xl font-bold text-white text-white text-white">Focus Engine</h2>
            <p className="mt-2 text-sm text-slate-500 text-slate-500 text-slate-400">Deep work session for maximum retention</p>
            
            <div className="mt-10 mb-8">
              <span className="text-7xl font-mono font-bold tracking-tighter text-white text-white text-white">
                {formatTime(timeLeft)}
              </span>
            </div>
            ...
            <div className="flex justify-center gap-4">
              {!isActive ? (
                <button
                  ...
                  className="flex items-center gap-2 rounded-2xl bg-brand px-8 py-4 text-lg font-bold text-white text-white text-white shadow-[0_0_30px_rgba(124,92,255,0.3)] transition hover:scale-105 hover:bg-brand/90"
                >
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className={cn(
                      ...
                      isPaused 
                        ? "bg-brand text-white text-white text-white shadow-glow" 
        ...
        <aside className="space-y-6">
          <div className="rounded-3xl border border-white/10 border-white/10 border-white/10 bg-white/[0.03] p-6 flex flex-col h-[600px]">
            <div className="flex items-center gap-3 mb-6">
              <div className="rounded-lg bg-cyan/10 p-2 text-cyan">
                <MessageSquare size={20} />
              </div>
              <h2 className="font-bold text-white text-white text-white">Task Assistant</h2>
            </div>
            
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
              <div className="rounded-2xl bg-white/[0.02] border border-white/5 p-4 text-sm text-slate-300 text-slate-300 text-slate-300 leading-relaxed">
              ...
              {currentConversation?.messages.filter(m => m.role !== 'system').map((msg, i) => (
                <div key={i} className={cn(
                  ...
                  msg.role === 'user' 
                    ? "ml-auto bg-brand/20 text-white text-white text-white border border-brand/20" 
                    : "bg-white/[0.02] text-slate-300 text-slate-300 text-slate-300 border border-white/5"
                )}>
                  {msg.content}
                </div>
              ))}
            ...
            <div className="mt-6 pt-4 border-t border-white/5 space-y-4">
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-white/[0.05] border border-white/10 border-white/10 border-white/10 rounded-xl px-4 py-3 text-sm text-white text-white text-white placeholder-slate-500 focus:outline-none focus:border-brand/50 transition-all"
                  onKeyDown={(e) => {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="text-center">
                ...
                <h2 className="text-2xl font-bold text-white text-white text-white">How confident do you feel?</h2>
                <p className="mt-2 text-slate-500 text-slate-500 text-slate-400 text-sm">Be honest - this tunes Veda's memory engine to optimize your next review session.</p>
              </div>
              ...
              <div className="mt-8 grid grid-cols-5 gap-3">
                {[1, 2, 3, 4, 5].map((score) => (
                  ...
                  >
                    <div className="w-full aspect-square rounded-2xl border border-white/10 border-white/10 border-white/10 bg-transparent bg-transparent bg-white/5 flex items-center justify-center text-xl font-bold text-slate-500 text-slate-500 text-slate-400 transition-all group-hover:bg-brand group-hover:text-white text-white text-white group-hover:border-brand group-hover:scale-110">
                      {score}
                    </div>
              ...
              <button 
                onClick={() => setShowConfidence(false)}
                className="mt-8 w-full py-3 text-sm font-medium text-slate-500 hover:text-white text-white text-white transition"
              >
                Skip Assessment
              </button>
            </motion.div>
