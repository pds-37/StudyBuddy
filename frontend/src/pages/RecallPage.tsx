...
export function RecallPage() {
  ...
  const submitReview = async (grade: RecallGrade) => {
    ...
    try {
      setReviewing(true);
      setError(null);
      // 'answer' text is no longer used, passing empty string for backend compat
      const review = await reviewRecallAnswer({ noteId: activePrompt.noteId, answer: "", grade });
      setResult(review);
      setShowResult(true);
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs mt-1.5 font-medium leading-normal">
                {focusedNoteId
                  ...
                  : "Active recall triggers neuroplasticity. Flashcard mode engaged."}
              </p>
            </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.06] pb-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-mono mt-0.5">SM-2 Interval Calculated</p>
                    </div>
                  </div>
                ...
                <div className="space-y-4 pt-4 border-t border-white/10">
                  {!showAnswer ? (
                    <button
                      ...
                      className="w-full py-6 rounded-xl border border-brand/30 bg-brand/10 text-brand-light hover:bg-brand/20 transition-all font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2"
                    >
                      <Eye size={16} /> Reveal Answer
                    </button>
                  ) : (
                    <div className="space-y-6">
                      {/* Visual & Text Expected Answer */}
                      <div className="space-y-4">
                        <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-widest font-mono flex items-center gap-2">
                        ...
                        {/* Text Answer */}
                        {activePrompt.expectedAnswer && (
                          <div className="text-sm text-slate-300 leading-relaxed bg-white/[0.02] border border-white/[0.05] p-4 rounded-xl">
                            {activePrompt.expectedAnswer}
                          </div>
                        )}

                        {/* Visual/Diagram Answer */}
                        {(activePrompt.answerImageUrl || activePrompt.answerDiagram) && (
                          <div className="rounded-xl border border-white/10 bg-slate-950/50 p-4 overflow-hidden relative group">
                            {activePrompt.answerImageUrl && (
                              <img src={activePrompt.answerImageUrl} alt="Expected visual answer" className="w-full rounded-lg border border-white/5 shadow-2xl mb-4 last:mb-0" />
                            )}
                            {activePrompt.answerDiagram && (
                              <div className="font-mono text-xs text-brand-light bg-brand/5 p-4 rounded-lg overflow-x-auto border border-brand/10">
                                <pre><code>{activePrompt.answerDiagram}</code></pre>
                                <p className="mt-2 text-[9px] text-slate-500 italic">Mermaid Flowchart</p>
                              </div>
                            )}
                          </div>
                      ...
                      <div className="flex flex-col gap-3 pt-4 border-t border-white/10">
                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest font-mono text-center mb-1">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <button
                            ...
                            className="py-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-xs font-black uppercase tracking-wider hover:bg-emerald-500/20 hover:shadow-[0_0_15px_rgba(16,185,129,0.15)] transition-all disabled:opacity-40 flex flex-col items-center gap-1.5"
                          >
                          ...
                          <button
                            ...
                            className="py-4 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-400 text-xs font-black uppercase tracking-wider hover:bg-amber-500/20 hover:shadow-[0_0_15px_rgba(245,158,11,0.15)] transition-all disabled:opacity-40 flex flex-col items-center gap-1.5"
                          >
                            <span className="text-lg">📐</span>
                            Partial
                          </button>
                          <button
                            ...
                            className="py-4 rounded-xl border border-red-500/30 bg-red-500/10 text-red-400 text-xs font-black uppercase tracking-wider hover:bg-red-500/20 hover:shadow-[0_0_15px_rgba(239,68,68,0.15)] transition-all disabled:opacity-40 flex flex-col items-center gap-1.5"
                          >
                            <span className="text-lg">💀</span>
                            Forgot
                          </button>
