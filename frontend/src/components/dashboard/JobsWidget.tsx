    ...
    <Card className="p-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white text-white text-white">Job opportunities</h3>
            <p className="text-sm text-slate-500 text-slate-500 text-slate-400">Live market roles matched to your profile</p>
          </div>
        </div>
        <Link
          to="/jobs"
      ...
      {loading ? (
        ...
        <div className="mt-6 space-y-5">
          <div className="rounded-[1.5rem] border border-white/8 bg-black/20 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-3 font-display text-4xl tracking-tight text-white text-white text-white">{jobs.length}</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-slate-500 text-slate-500 text-slate-400">{totalMatches} with a match score</p>
          ...
          <div className="rounded-[1.35rem] border border-white/8 bg-white/[0.03] p-4">
            ...
            {topJob ? (
              <>
                <p className="mt-3 text-base font-semibold text-white text-white text-white">
                  {topJob.title} at {topJob.company}
                </p>
                <p className="mt-2 text-sm text-slate-500 text-slate-500 text-slate-400">
                  {topJob.location} · {topJob.matchScore}% match{topJob.source ? ` · ${topJob.source}` : ""}
                </p>
              </>
            ) : (
              <p className="mt-3 text-sm leading-7 text-slate-500 text-slate-500 text-slate-400">
