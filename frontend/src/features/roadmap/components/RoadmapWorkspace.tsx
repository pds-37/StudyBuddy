...
export function RoadmapWorkspace() {
  ...
  // View toggle: "execution" vs "curriculum" vs "galaxy"
  const [currentView, setCurrentView] = useState<"execution" | "curriculum" | "galaxy">("execution");
  const [selectedGalaxyNode, setSelectedGalaxyNode] = useState<any>(null);
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_340px] relative z-10">
        <main className="min-w-0 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pb-4 border-b border-white/[0.06]">
            ...
            {/* Premium Three-Mode Tab Switcher */}
            <div className="flex items-center gap-1 bg-white/[0.03] border border-white/10 rounded-xl p-1 shrink-0 self-start sm:self-auto shadow-inner">
              <button
                ...
                className={cn(
                  "rounded-lg px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-all duration-300",
            ...
            {currentView === "galaxy" && (
              ...
              >
                <ThreeSkillGalaxy
                  ...
                  selectedNode={selectedGalaxyNode}
                  onNodeClick={(node) => {
                    setSelectedGalaxyNode(node);
                    if (node.type === "task") {
                      setSelectedTaskId(node.id);
                    } else if (node.type === "phase") {
                      setActivePhaseId(node.id);
