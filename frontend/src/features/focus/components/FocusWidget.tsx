...
export function FocusWidget() {
  ...
  if (!isActive) {
    return (
      <button
        ...
        className="group flex items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-1.5 text-xs font-medium text-slate-300 transition hover:bg-white/[0.06]"
      >
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-brand/30 bg-brand/10 px-3 py-1.5 shadow-[0_0_20px_rgba(124,92,255,0.15)] relative overflow-hidden">
      ...
      <button
        onClick={stopSprint}
        className="relative z-10 ml-2 rounded-md hover:bg-white/10 p-0.5 text-slate-300 transition"
        title="Stop Sprint"
      >
