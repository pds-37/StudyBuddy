    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <Motion.div
          ...
          className="relative w-full max-w-2xl bg-transparent bg-obsidian border border-white/[0.06] rounded-[40px] shadow-2xl overflow-hidden"
        >
          {/* HEADER */}
          <div className="p-8 border-b border-white/[0.04] flex items-center justify-between bg-gradient-to-r from-brand/5 to-transparent">
            <div>
              ...
              <h2 className="text-2xl font-bold text-white text-white tracking-tight">{job.title}</h2>
          ...
          <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
            {/* MATCH SCORE HERO */}
            <div className="flex items-center gap-8 p-8 rounded-3xl bg-transparent bg-white/[0.02] border border-white/[0.04]">
              <div className="relative w-24 h-24 flex items-center justify-center">
                 <svg className="w-full h-full -rotate-90">
                   <circle cx="48" cy="48" r="44" fill="none" stroke="currentColor" strokeWidth="8" className="text-slate-100 text-white/5" />
                   <Motion.circle 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {job.requiredSkills.slice(0, 3).map((skill: string) => (
                    <div key={skill} className="flex items-center gap-2 text-sm text-slate-300 text-slate-300 font-medium">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-sm text-slate-300 text-slate-300 font-medium opacity-60">
          ...
          {/* FOOTER ACTIONS */}
          <div className="p-8 border-t border-white/[0.04] bg-transparent bg-white/[0.01] flex gap-4">
            <button className="flex-1 py-4 rounded-2xl bg-brand text-white text-white text-[11px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg hover:shadow-brand/20 transition-all">
              <Rocket className="w-4 h-4" /> Apply Now
            </button>
            <button className="flex-1 py-4 rounded-2xl bg-white/5 border border-white/10 text-white text-white text-[11px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all">
              Add to Prep Roadmap
            </button>
          </div>
        </Motion.div>
