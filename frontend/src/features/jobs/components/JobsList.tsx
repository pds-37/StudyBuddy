...
export function JobsList({ refreshTrigger }: JobsListProps) {
  ...
  const getMatchScoreColor = (score?: number) => {
    if (!score) return "text-slate-500 text-slate-500 text-slate-400";
  ...
  if (loading && jobs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 animate-fade-in">
        ...
        <p className="mt-6 text-slate-500 text-slate-500 text-slate-400 font-medium">Scanning live market opportunities...</p>
      </div>
    );
  }
      ...
      <div className="rounded-[2rem] glass border-red-500/20 bg-red-500/5 p-10 animate-slide-up">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex-1 text-center md:text-left">
            <h3 className="text-2xl font-bold text-white text-white text-white mb-3">Market Sync Interrupted</h3>
            <p className="text-slate-500 text-slate-500 text-slate-400 max-w-md leading-relaxed">{error}</p>
          </div>
          <div className="flex items-center gap-4">
            <button
              onClick={handleClearError}
              className="px-6 py-3 rounded-xl bg-transparent bg-transparent bg-white/5 border border-white/10 border-white/10 border-white/10 text-white text-white text-white font-bold hover:bg-transparent bg-transparent bg-white/10 transition-all"
            >
              Dismiss
            </button>
            <button
              onClick={handleRefresh}
              className="flex items-center gap-2 px-6 py-3 rounded-xl bg-brand text-white text-white text-white font-bold hover:scale-105 transition-all shadow-[0_0_20px_rgba(124,92,255,0.4)]"
            >
  ...
  if (jobs.length === 0) {
    return (
      <div className="py-24 text-center glass rounded-[3rem] border-white/5 bg-white/[0.01] animate-slide-up">
        <div className="w-20 h-20 rounded-3xl bg-transparent bg-transparent bg-white/5 flex items-center justify-center text-slate-400 mx-auto mb-8">
          <Building size={40} />
        </div>
        <h3 className="text-2xl font-bold text-white text-white text-white mb-3">No live matches found</h3>
        <p className="text-slate-500 text-slate-500 text-slate-400 mb-10 max-w-sm mx-auto leading-relaxed">
          We couldn't find active openings matching your current profile. Try adjusting your target role or location.
        </p>
        <button
          onClick={handleRefresh}
          className="mx-auto flex items-center gap-2 px-8 py-4 rounded-2xl bg-transparent text-obsidian font-black hover:bg-slate-200 transition-all hover:scale-105"
        >
    ...
    <div className="space-y-10 animate-fade-in">
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <div className="p-6 rounded-2xl glass border-white/5 bg-white/[0.02] relative overflow-hidden group">
          ...
          <p className="text-4xl font-black text-white text-white text-white">{jobs.length}</p>
        </div>
        <div className="p-6 rounded-2xl glass border-white/5 bg-white/[0.02] relative overflow-hidden group">
          ...
          <p className="text-4xl font-black text-white text-white text-white">{freshJobs}</p>
        </div>
        <div className="p-6 rounded-2xl glass border-white/5 bg-white/[0.02] relative overflow-hidden group">
          ...
          <p className="text-4xl font-black text-white text-white text-white">{Object.keys(sourceCounts).length}</p>
        </div>
        <div className="p-6 rounded-2xl glass border-white/5 bg-white/[0.02] relative overflow-hidden group">
          ...
          <p className="text-4xl font-black text-white text-white text-white">{remoteJobs}</p>
        </div>
      </div>
            ...
            <div key={job.id} className="group p-8 rounded-[2.5rem] glass border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-all hover:border-white/10 border-white/10 border-white/10 hover:translate-y-[-2px]">
              <div className="flex flex-col lg:flex-row gap-8">
                <div className="flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                         <h3 className="text-2xl font-bold text-white text-white text-white group-hover:text-brand transition-colors">{job.title}</h3>
                         {job.isRemote && (
                      ...
                      <div className="flex flex-wrap items-center gap-6 text-sm text-slate-500 text-slate-500 text-slate-400">
                        <div className="flex items-center gap-2">
                          <Building size={16} className="text-slate-400" />
                          <span className="font-semibold text-slate-300 text-slate-300 text-slate-300">{job.company}</span>
                        </div>
                        <div className="flex items-center gap-2">
                  ...
                  <p className="text-slate-500 text-slate-500 text-slate-400 text-sm leading-relaxed mb-8 max-w-4xl line-clamp-2 group-hover:line-clamp-none transition-all">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-6">
                    <div className="flex flex-wrap gap-2">
                      {job.requiredSkills.slice(0, 5).map((skill) => (
                        <span
                          key={skill}
                          className="px-3 py-1.5 rounded-xl bg-transparent bg-transparent bg-white/5 border border-white/5 text-xs font-medium text-slate-300 text-slate-300 text-slate-300"
                        >
                          {skill}
                        </span>
                      ))}
                      {job.requiredSkills.length > 5 && (
                        <span className="px-3 py-1.5 rounded-xl bg-transparent bg-transparent bg-white/5 text-xs font-medium text-slate-500">
