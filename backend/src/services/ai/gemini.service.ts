...
async function requestGemini(messages: OpenAICompatibleMessage[], model: string = "gemini-1.5-flash", jsonMode: boolean = true) {
  ...
  // Translate messages to Gemini contents structure
...
/** Generates a career learning roadmap using Gemini. */
...
6. Provide ONLY valid JSON.`;
  ...
  const response = await requestGemini(oaiMessages, "gemini-1.5-flash", true);
  try {
...
/** Analyzes a note to extract structured concepts. */
...
/** Helper parsers for Resume Tailoring */
...
${resumeText.substring(0, 5000)}`;
  ...
  const response = await requestGemini([{ role: "user", content: prompt }], "gemini-1.5-flash", true);
  try { return JSON.parse(extractJsonPayload(response)); } catch { return { skills: [], experience: [] }; }
}
...
${jdText.substring(0, 5000)}`;
  ...
  const response = await requestGemini([{ role: "user", content: prompt }], "gemini-1.5-flash", true);
  try { return JSON.parse(extractJsonPayload(response)); } catch { return { required_skills: [], keywords: [] }; }
}
...
${JSON.stringify(parsedJD)}`;
  ...
  const response = await requestGemini([{ role: "user", content: prompt }], "gemini-1.5-flash", true);
  try { return JSON.parse(extractJsonPayload(response)); } catch { return { missing_required_skills: [], missing_keywords: [] }; }
}
...
/** Tailors a resume for a specific job using Gemini. */
...
RESUME MODE: ${request.mode || "technical"}
...
Return ONLY valid JSON. No commentary.`;
  ...
  const response = await requestGemini([
    { role: "system", content: "You return strict JSON for resume tailoring. Never include markdown fences or commentary." },
...
/** Generates skill intelligence report. */
...
2. Ensure dimensions (confidence, retention, etc.) reflect the USER CONTEXT provided.
